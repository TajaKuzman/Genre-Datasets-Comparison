...
def calculate_hits(extended_corpus, genre_distribution, classifiers):
	...
	# Print out a df
	results_df = pd.DataFrame(classifier_comparison_dict).sort_values("Hits percentage", ascending=False)
	print(results_df.to_markdown(index=False))

# Extend this to calculate number of hits for each category
def calculate_hits_categories(extended_corpus, genre_distribution, classifiers):
	# Create a list of domains from the df that is domain-level (genre_distribution)
	domain_list = list(genre_distribution["domain_id"].unique())

	for classifier in classifiers:
		classifier_comparison_dict = {"{}".format(classifier): [], "Hits": [], "Category support": [], "Number of pairs with category": [], "Hits per pairs": []}

		# Get the list of categories per classifier
		categories = list(extended_corpus[classifier].unique())

		# Calculate hits for each category
		for category in categories:
			classifier_comparison_dict["{}".format(classifier)].append(category)
			category_counter = 0
			hits = 0
			pair_number = 0
		# For each domain, calculate the hits:
			...
			classifier_comparison_dict["Hits"].append(hits)
			classifier_comparison_dict["Category support"].append(category_counter)
			classifier_comparison_dict["Number of pairs with category"].append(pair_number)
			classifier_comparison_dict["Hits per pairs"].append(round(hits/pair_number,3))

		# Print out a df
		results_df = pd.DataFrame(classifier_comparison_dict).sort_values(by="{}".format(classifier))
